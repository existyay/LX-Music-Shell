...
- 封面显示改为优先半块真彩渲染, 兼容 Kitty 在内的所有真彩终端
- 修复 Kitty 原生协议可能不显示图片的问题
- 播放页封面和文字布局自适应宽窄窗口
...
- Kitty 终端封面改为 PNG 原生图形协议, 更清晰
- 封面下载后自动 ffmpeg 转 PNG
- 播放页封面更大 (高度最高 14 行), 宽窗口下右侧排布歌曲信息
...
- 修复鼠标点击后显示变成两行的问题: 未识别的 ESC 序列不再当作普通字符
- 避免鼠标序列解析失败时污染搜索框内容
- ESC 序列读取超时增加到 0.03s, 鼠标事件解析更稳定
...
- 搜索框重构为现代圆角输入条 (╭ ╮), 聚焦/未聚焦状态清晰
- 输入事件响应延迟从 0.2s 降到 0.1s
- 搜索框光标显示在输入条内, 视觉更干净
